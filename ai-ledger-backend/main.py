...
class ParsedTransaction(BaseModel):
    ...
    amount: float = Field(..., description="The total transaction cost or amount. For 'adjustment' type, this must be the absolute current balance shown on the screen.")
    ...
    transaction_type: Literal["expense", "income", "transfer", "adjustment"] = Field(
        ..., 
        description="Type of the transaction. 'expense' for spending, 'income' for earnings, 'transfer' for moving funds between accounts (including credit card repayments), 'adjustment' for bank card balance snapshots."
    )
    ...
    to_account: Optional[VALID_ACCOUNTS] = Field(
        None, 
        description="The account where the funds flow TO. REQUIRED for 'income', 'transfer', and 'adjustment' types. Must be null for 'expense'."
    )
    ...
    remarks: str = Field(..., description="A summary of what was bought combined with user notes.")
...
async def record_transaction(payload: RecordRequest):
    ...
    try:
        ...
        if parsed_data.transaction_type == 'adjustment':
            ...
            msg = f"成功上帝平账：【{parsed_data.to_account}】当前水位校准为 ￥{parsed_data.amount:.2f} 元。"
        else:
            # 普通收支与转账
